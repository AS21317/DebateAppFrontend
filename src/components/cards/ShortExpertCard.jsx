...
const ShortExpertCard = ({expertData}) => {
    return <div className="flex bg-green-200  m-4  p-2 rounded-lg flex-row  gap-4">
      <div className="flex   items-center"> 
      <img src={expertData.user.photo ||demoImg } alt={"expertPic"} className="rounded-full" width={35}  />
      </div>
      
      <div className="flex flex-col  text-[16px] font-medium flex-wrap
       justify-between ">
        <p >{expertData.user.name} ( {expertData.user.role} )</p>
  
       <div className="flex flex-wrap gap-1">
          {expertData.user.expertise?.map((interest) => {
            return (
...
export default ShortExpertCard